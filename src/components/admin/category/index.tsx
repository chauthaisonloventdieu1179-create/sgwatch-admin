...
import { ICategory, ICategoriesResponse } from "@/types/admin/category";
...
const CategoryList = () => {
  ...
  const handleUpdate = (id: number) => {
    router.push(`/admin/category/update?id=${id}`);
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      if (!idDelete) return;
      await sendRequest({
        url: `/admin/shop-categories/${idDelete}`,
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
                ...
                >
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-[#E6E6E6] text-left text-[14px] text-[#0D1526] font-[500]">
                      ...
                      <th
                        style={{ padding: "12px 16px" }}
                        className="w-[22%] text-left text-[14px]"
                      >
                        Tên danh mục
                      </th>
                      <th
                        style={{ padding: "12px 16px" }}
                        className="w-[18%] text-left text-[14px]"
                      >
                        Slug
                      </th>
                      <th
                      ...
                      <th
                        style={{ padding: "12px 16px" }}
                        className="w-[16%] text-right text-[14px]"
                      >
                        Thao tác
                      </th>
                    </tr>
                    ...
                    {Array.isArray(categories) &&
                      categories.map((item, index) => (
                        ...
                        >
                          ...
                          >
                            {item.image_url ? (
                              <img
                                src={item.image_url}
                                alt={item.name}
                                className="w-[50px] h-[35px] object-cover rounded-[4px]"
                              />
                            ) : (
                          ...
                          >
                            {item.parent_id ? (
                              <span className="pl-[16px] text-[#555]">
                                ┗ {item.name}
                              </span>
                            ) : (
      ...
      {isModalVisible && (
        <div className="fixed inset-0 bg-[#000000] bg-opacity-20 z-[1500] flex justify-center items-center">
          <ConfirmDelete
            ...
            moduleText={"Bạn có chắc chắn muốn xóa danh mục này?"}
            confirm={"Danh mục"}
            id={nameDelete}
          />
        </div>
